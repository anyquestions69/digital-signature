<template>
	<div class="main-documents__body">
		...
		<ul class="document__list">
			<li v-for="elem in PostList" :key="elem.id" class="list__elem">
				...
				<div class="elem__tools" v-if="AuthStore.role === 'Admin'">
					<button title="изменить" @click="editPost(elem.id, elem.title)">
					...
					<button title="удалить" @click="delPost(elem.id)">
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
							<path
		...
		<button class="document__btn" @click="nextPage" v-if="scroll">
			Следующая...
		</button>
...
const page = ref(1)
const scroll = ref(PostStore.scroll)
const limit = 7
...
const delPost = async (postId: number) => {
	...
	PostStore.postList = PostStore.postList.filter(post => post.id !== postId)
...
const editPost = (postId: number, currentTitle: string) => {
