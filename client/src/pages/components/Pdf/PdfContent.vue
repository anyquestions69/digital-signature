		...
		<div>
			<div class="status__text" v-if="AuthStore.role === 'Guest'">
				<p>
					Статус:
					<span
						:style="`background: ${isSuccessStatus ? '#1bb28a' : '#77173c'}`"
					>
			...
			<div class="status__text" v-else>
				<p>
					...
					<span
						:style="`background: ${
							PostStore.post.delivered ? '#1bb28a' : '#77173c'
						}`"
					>
						{{ PostStore.post.delivered ? 'Подписан всеми' : 'Не подписан' }}
					</span>
				</p>
...
console.log(PostStore.post.delivered)
